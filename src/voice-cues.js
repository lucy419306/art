...
(function (root) {
  ...
  const cues = {
    ...
    'p4.meaning': shared('本次考核将以三项决策模拟，评估你的自主决策能力：如何生存，与谁亲密，以及如何面对无法删除的记忆。.mp3', '本次考核将以三项决策模拟题，评估你的自主决策能力：如何生存，与谁亲密，以及如何面对无法删除的记忆。'),
    ...
    'P5.right': shared('已记录。根据历史样本，该选择产生长期后悔的概率为 63%。.mp3', '已记录。根据历史样本，该选项产生长期后悔的概率为 63%。'),
    ...
    // 用户补录文件响度约高 2.8 dB；只校正播放电平，不改动原文件与速度。
    'P6.left': { ...shared('该匹配由系统预先安排。.mp3', '该匹配由系统预先安排。'), volume: 0.72 },
  ...
  for (const id of ['b', 'c']) {
    ...
    // 用户补录文件响度约高 1.5 dB，与前后两句对齐。
    cues[id + '.pain'] = { ...shared('系统将不再替你删除所有痛苦。.mp3', '系统将不再替你删除所有痛苦。'), volume: 0.84 };
